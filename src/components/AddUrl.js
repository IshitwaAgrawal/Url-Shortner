...
const AddUrl = ({ id, jwt, urls_changed }) => {
  ...
  const addUrl_handler = () => {
    ...
    axios
      .post(endpoint.addUrl + uuid, url_data, {
        headers: {
  ...
  return (
    <div className="addurl">
      <input
        className="addurl__input"
        type="text"
        value={url}
        onChange={urlChangeHandler}
      />
      <button className="addurl__button" onClick={addUrl_handler}>
        Add
      </button>
    </div>
  );
...
const mapDispatchToProps = (dispatch) => ({
  urls_changed: (urls) =>
    dispatch({ type: actions.change_urls, payload: urls }),
});
